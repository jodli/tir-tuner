...
`load → window → glycemic → meals → corrections → settings → snapshot →
recommend → clamp → history`, then report + charts. Each stage is a pure
`run(state, config) -> state`, so any stage can be re-run from a saved artifact:
```sh
...
per unit. See `settings.example.json`. This file is git-ignored.
